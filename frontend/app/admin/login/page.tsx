      ...
      <div className="w-full max-w-md bg-white rounded-lg shadow-md p-10 md:p-12">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input type="email" className="w-full border-2 border-gray-300 rounded-lg px-5 py-3.5" value={email} onChange={e=>setEmail(e.target.value)} required />
          </div>
          <div>
            <label className="block text-sm font-medium text-black mb-3">Password</label>
            <input type="password" className="w-full border-2 border-gray-300 rounded-lg px-5 py-3.5" value={password} onChange={e=>setPassword(e.target.value)} required />
          </div>
          <div className="flex items-center justify-between pt-2">
            <button type="submit" className="bg-black text-white px-10 py-3.5 rounded-lg font-semibold">Login</button>
          </div>
        </form>
